...
interface EffectFrame extends RectFrame {
  radius?: number;
  opacity?: number;
}

type LabelFrame = Omit<RectFrame, 'height'> & {
  height?: number;
};
...
interface GooeyNavProps {
  ...
  particleR?: number;
  timeVariance?: number;
  colors?: readonly ParticleColor[];
  initialActiveIndex?: number;
  activeKey?: string;
  className?: string;
  onItemSelect?: (item: GooeyNavItem) => void;
}
...
const defaultColors: readonly ParticleColor[] = [1, 2, 3, 1, 2, 3, 1, 4];
...
const applyFrame = (element: HTMLElement | null, frame: EffectFrame | null, immediate = false) => {
...
export default function GooeyNav({
  ...
  const updateEffectPosition = (element: HTMLElement | null, item: GooeyNavItem, immediate = false) => {
  ...
  const handleSelect = (event: MouseEvent<HTMLButtonElement> | KeyboardEvent<HTMLButtonElement>, index: number) => {
  ...
  useEffect(() => {
    ...
    const activeButton = navRef.current.querySelectorAll<HTMLElement>('[data-gooey-nav-item]')[resolvedActiveIndex];
    ...
    const resizeObserver = new ResizeObserver(() => {
      const currentButton = navRef.current?.querySelectorAll<HTMLElement>('[data-gooey-nav-item]')[resolvedActiveIndex];
